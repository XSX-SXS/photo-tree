import { useState, useMemo, useRef, useEffect, Suspense, Component, ErrorInfo, ReactNode } from 'react';
...
class ErrorBoundary extends Component<{ children: ReactNode }, { hasError: boolean, error: Error | null }> {
  ...
  render() {
    if (this.state.hasError) {
      return (
        <div style={{ width: '100vw', height: '100vh', display: 'flex', justifyContent: 'center', alignItems: 'center', backgroundColor: '#000', color: '#ff0000' }}>
          <div>
            <h1>Canvas Error</h1>
            <p>{this.state.error?.message}</p>
            <button 
              onClick={() => this.setState({ hasError: false, error: null })} 
              style={{ padding: '10px 20px', marginTop: '20px', backgroundColor: '#ffd700', color: '#000', border: 'none', cursor: 'pointer' }}
            >
              Retry
            </button>
          </div>
